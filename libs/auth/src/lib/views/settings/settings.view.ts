...
@Component({
  template: `
    <h1>Settings</h1>
    <ngbs-auth-update-email-form-widget (action$)="action$.next($event)"> </ngbs-auth-update-email-form-widget>
...
export class NgbsAuthSettingsView {
  ...
  public readonly dispatchAction = this.action$.subscribe(action => this.store.dispatch(action))
}
